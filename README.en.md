...
- When installing the system package, ensure that the openssl version on the system is 1.1. Users of x86_ 64 need to install openssl-devel version 1.1. Users can install it through internal source, EPEL release, etc. Please confirm that the source meets the organization's security requirements before installation. When using offline installation, users can modify C_INCLUDE_Path environment variable and other methods to confirm that openssl-devel that meets the requirements can be found.You can refer to `declare -x C_INCLUDE_PATH="/usr/include/openssl11"` and `declare -x OPENSSL_ROOT_DIR="/usr/lib64/openssl11"` to configure environment variables.Users of aarch64 need to install openssl version 1.1. Please refer to [reference link](https://www.openssl.org/source/), after obtaining the software package, compile and install it. Please refer to [reference link]( https://wiki.openssl.org/index.php/Compilation_and_Installation)。 After installation, you can refer to `export C_INCLUDE_Path=/usr/local/openssl/include` and `declare -x OPENSSL_ROOT_DIR="/usr/local/openssl/lib"` to configure environment variables to confirm that openssl that meets the requirements can be found.
